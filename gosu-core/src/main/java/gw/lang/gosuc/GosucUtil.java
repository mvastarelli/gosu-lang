...
public class GosucUtil {
  ...
  public static IDirectory getDirectoryForPath( String uriPath ) {
    return CommonServices.INSTANCE.getFileSystem().getDirectory( Paths.get( URI.create( uriPath ) ) );
  }
  ...
  public static List<IDirectory> toDirectories(List<String> uriPaths) {
    ...
    for (String uri : uriPaths) {
      dirs.add( CommonServices.INSTANCE.getFileSystem().getDirectory( Paths.get( URI.create( uri ) ) ) );
    }
    return dirs;
  }
